...
def warp_image(src, dst, h_matrix):
    ...
    dst = cv2.remap(src, map_x, map_y, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_TRANSPARENT, dst=dst)
...
def compute_affine_rectification(src_img:np.ndarray,lines_vec: list):
    ...
    # Get the new size of the destination
    M = []
    ...
    w_new, h_new = np.max(transformed, axis=0)
    
    dst = np.zeros((int(h_new), int(w_new), 3))
...
def compute_homography_ransac(src, dst, thresh=16.0, num_tries=200):
    ...
    for _ in range(num_tries):
        ...
        random_indices = np.random.randint(0, len(src), size=4)
        src_sample = src[random_indices, :]
        dst_sample = dst[random_indices, :]
        ...
        inliers = np.sum(np.where(d < thresh, True, False))
