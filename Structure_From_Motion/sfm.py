...
def get_init_extrinsics(image_id1: str, image_id2: str, intrinsics: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    _, R, t, mask = cv2.recoverPose(essential_mtx, points2d_1, points2d_2, intrinsics, R, t, mask)
...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    for i, pt_3d in enumerate(points3d):
        reprojected_pt2d = homography @ np.vstack([pt_3d.reshape(3, 1), [1]])
        residuals[i] = calc_dist(points2d[i], reprojected_pt2d/reprojected_pt2d[-1])
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        """
        rvec = None
        _, rvec, tvec = cv2.solvePnP(selected_pts3d, selected_pts2d, intrinsics, distCoeffs=None, rvec=rvec, tvec=tvec, flags=cv2.SOLVEPNP_ITERATIVE, useExtrinsicGuess=False)
...
def add_points3d(image_id1: str, image_id2: str, all_extrinsic: dict, intrinsics, points3d: np.ndarray,
    ...
    """
    new_points3d = triangulate(image_id1, image_id2, kp_idxs1=matches[:, 0], kp_idxs2=matches[:, 1], extrinsics1=all_extrinsic[image_id1], 
