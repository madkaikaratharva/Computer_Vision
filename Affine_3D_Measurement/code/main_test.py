...
def detect_lines(image: np.ndarray) -> np.ndarray:
    ...
    edge_image = cv2.Canny(image, threshold1=MIN_CANNY_THRESHOLD, threshold2=MAX_CANNY_THRESHOLD)
    
    line_pts = cv2.HoughLinesP(image=edge_image, rho=1, theta=(np.pi/180), threshold=100,
...
def get_support_mtx(intersections: np.ndarray, lines: np.ndarray, distance_threshold: float = DISTANCE_THRESHOLD):
    ...
    for i in range(num_intersections):
        for j in range(num_lines):
            ...
            dst = np.linalg.norm(intersections[i] - lines[j])
...
def get_vanishing_pts(lines: np.ndarray, num_vanishing_pts: int) -> np.ndarray:
    ...
    for _ in range(num_vanishing_pts):
        ...
        pt_idx = np.argmax(np.sum(support_mtx, axis=1))
        
        # Get the vanishing point
        u, v, w = intersections[pt_idx]
        vanishing_pts.append((u, v, w))
        
        # Get the set of support lines
        line_idx = np.where(support_mtx[pt_idx] == 1)
        
        # Remove the support lines from M
        support_mtx[:, line_idx] = 0
...
def get_target_height(vanishing_line: np.ndarray, query_info: dict, target_info: dict, vanishing_pt_v: np.ndarray):
    ...
    """
    # Vanishing Point u
    #line_1 = np.cross(pt_b1, pt_b2)
    #line_1 = line_1 / line_1[2]
    
    ##pt_u = np.cross(line_1, vanishing_line)
    #pt_u = pt_u / pt_u[2]
    
    # Transferred Point
    #line_2 = np.cross(pt_t1, pt_u)
    #line_2 = line_2 / line_2[2]
    
    #line_3 = np.cross(vanishing_pt_v, pt_b2)
    #line_3 = line_3 / line_3[2]
    
    #transferred_pt = np.cross(line_2, line_3)
    #transferred_pt = transferred_pt / transferred_pt[2]
    
    # Distances
    #dst_transferred_pt = np.linalg.norm(pt_b2 - transferred_pt)
    #dst_t2 = np.linalg.norm(pt_b2 - pt_t2)
    #dst_v = np.linalg.norm(pt_b2 - vanishing_pt_v)
    
    # Distance Ratio
    #distance_ratio = (dst_transferred_pt * (dst_v - dst_t2)) / (dst_t2 * (dst_v - dst_transferred_pt))
